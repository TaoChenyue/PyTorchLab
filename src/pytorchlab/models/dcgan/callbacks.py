...
class DCGANCallback(Callback):
    def __init__(
        self,
        latent_dim: int,
        nums: int = 8,
        tag: str = "dcgan_images",
        **kwargs,
    ):
        ...
        Args:
            ...
            tag(str, optional): tag name of directory for saving images. Default to "dcgan_images".
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        z = torch.randn((self.nums, self.latent_dim, 1, 1)).to(pl_module.device)
        with torch.no_grad():
            pl_module.eval()
            images = pl_module(z)
            pl_module.train()
