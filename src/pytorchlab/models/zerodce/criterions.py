...
def spa_loss(org: torch.Tensor, enhance: torch.Tensor):
    device = org.device
    kernel_left = (
    ...
    weight_diff = torch.max(
        torch.FloatTensor([1]).to(device=device)
        + 10000
        * torch.min(
            org_pool - torch.FloatTensor([0.3]).cuda(),
            torch.FloatTensor([0]).cuda(),
        ),
        torch.FloatTensor([0.5]).to(device=device),
    )
    E_1 = torch.mul(
        torch.sign(enhance_pool - torch.FloatTensor([0.5]).to(device=device)),
...
def exp_loss(x: torch.Tensor, patch_size: float, mean_val: float):
...
def TV_loss(x: torch.Tensor, TVLoss_weight: float = 1):
    batch_size = x.size()[0]
    h_x = x.size()[2]
    w_x = x.size()[3]
