...
class DCGAN(LightningModule):
    ...
    def test_step(self,batch: Sequence[torch.Tensor],batch_idx: int,dataloader_idx: int=0) -> torch.Tensor:
        x=batch[0]
        batch_size = x.size(0)
        # Sample noise
        z = torch.randn(batch_size, self.latent_dim).to(self.device)
    ...
    def predict_step(self,batch: Sequence[torch.Tensor],batch_idx: int,dataloader_idx: int=0) -> torch.Tensor:
        x=batch[0]
        batch_size = x.size(0)
        # Sample noise
        z = torch.randn(batch_size, self.latent_dim).to(self.device)
